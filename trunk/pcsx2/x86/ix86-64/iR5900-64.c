...
// 64 bit recompiler - difference between 32bit is that mmx is gone
// and x86 regs are now cached (also fancier block linking) (zerofrog)
...
extern void rpropBSC(EEINST* prev, EEINST* pinst);
...
void _eeFlushAllUnused()
{
	...
	for(i = 0; i < X86REGS; ++i) {
		if( x86regs[i].inuse && x86regs[i].type != X86TYPE_GPR )
			_freeX86reg(i);
	}
}
...
u32* _eeGetConstReg(int reg)
{
	...
	if( _recIsRegWritten(g_pCurInstInfo+1, (s_nEndBlock-pc)/4, XMMTYPE_GPRREG, reg) ) {
		u64* ptempmem;
		ptempmem = (u64*)recAllocStackMem(8, 4);
		ptempmem[0] = g_cpuConstRegs[ reg ].UD[0];
		return (u32*)ptempmem;
	}
...
int _flushX86unused()
{
	int i;
	for (i=0; i<X86REGS; i++) {
		if (!x86regs[i].inuse || x86regs[i].needed || !(x86regs[i].mode&MODE_WRITE) ) continue;
		
		if (x86regs[i].type == XMMTYPE_GPRREG ) {
			//if( !(g_pCurInstInfo->regs[x86regs[i].reg]&EEINST_USED) ) {
			if( !_recIsRegWritten(g_pCurInstInfo+1, (s_nEndBlock-pc)/4, XMMTYPE_GPRREG, x86regs[i].reg) ) {
				_freeX86reg(i);
				x86regs[i].inuse = 1;
				return 1;
			}
...
int _flushUnusedConstReg()
{
	...
	for(i = 1; i < 32; ++i) {
		if( (g_cpuHasConstReg & (1<<i)) && !(g_cpuFlushedConstReg&(1<<i)) &&
            ...
            // check if can get away with sign extending
            if( (g_cpuConstRegs[i].SL[0]>>31) == g_cpuConstRegs[i].SL[1] )
                MOV64I32toM((uptr)&cpuRegs.GPR.r[i].UD[0], g_cpuConstRegs[i].UL[0]);
            else {
...
void _flushConstReg(int reg)
{
	if( GPR_IS_CONST1( reg ) && !(g_cpuFlushedConstReg&(1<<reg)) ) {
        ...
        // check if can get away with sign extending
        if( (g_cpuConstRegs[reg].SL[0]>>31) == g_cpuConstRegs[reg].SL[1] )
            MOV64I32toM((uptr)&cpuRegs.GPR.r[reg].UD[0], g_cpuConstRegs[reg].UL[0]);
        else {
	...
	for(i = 1; i < 32; ++i) {
		if( g_cpuHasConstReg & (1<<i) ) {
			...
			if( !(g_cpuFlushedConstReg&(1<<i)) ) {
				...
				u8* ptemp[3];
				CMP32ItoM((uptr)&cpuRegs.GPR.r[i].UL[0], g_cpuConstRegs[i].UL[0]);
				ptemp[0] = JNE8(0);
				if( EEINST_ISLIVE1(i) ) {
					CMP32ItoM((uptr)&cpuRegs.GPR.r[i].UL[1], g_cpuConstRegs[i].UL[1]);
					ptemp[1] = JNE8(0);
				}
				ptemp[2] = JMP8(0);
...
void* recAllocStackMem(int size, int align)
{
	...
	if( (uptr)recStackPtr % align ) recStackPtr += align - ((uptr)recStackPtr%align);
	recStackPtr += size;
...
void _deleteEEreg(int reg, int flush)
{
	...
	GPR_DEL_CONST(reg);
    _deleteX86reg(X86TYPE_GPR, reg, flush ? 0 : 2);
...
#define PROCESS_EE_SETMODES(mmreg) 0//((mmxregs[mmreg].mode&MODE_WRITE)?PROCESS_EE_MODEWRITES:0)
#define PROCESS_EE_SETMODET(mmreg) 0//((mmxregs[mmreg].mode&MODE_WRITE)?PROCESS_EE_MODEWRITET:0)
	...
	if( xmminfo & XMMINFO_WRITED ) {
		...
		if( regd < 0 ) {
			if( !(xmminfo&XMMINFO_READD) && (xmminfo & XMMINFO_READT) && (_Rt_ == 0 || (g_pCurInstInfo->regs[_Rt_] & EEINST_LASTUSE) || !EEINST_ISLIVEXMM(_Rt_)) ) {
				_freeXMMreg(EEREC_T);
                _deleteX86reg(X86TYPE_GPR, _Rd_, 2);
...
int recInit( void ) 
{
	...
	startaddr = 0x0d000000;
	while(!(startaddr & 0xf0000000)) {
		recMem = (char*)SysMmap(startaddr, REC_CACHEMEM);
...
{
	int left = 4 - ((uptr)p % 16)/sizeof(BASEBLOCK);
	recClearMem(p);
...
{
	int left = 4 - ((uptr)p % 32)/sizeof(BASEBLOCK);
	recClearMem(p);
...
void recClearMem(BASEBLOCK* p)
{
	...
	JMP32((uptr)DispatcherClear - ( (uptr)x86Ptr + 5 ));
...
//				SSE_MOVSS_XMM_to_M32((uptr)&cpuRegs.pc, mmreg);
//			}
//			else {
//				MOV32MtoR(EAX, (uptr)&cpuRegs.GPR.r[ reg ].UL[ 0 ] );
...
void SetBranchImm( u32 imm )
{
	...
	MOV32ItoR(EDX, 0);
	ptr = (u32*)(x86Ptr-4);
	*ptr = (u32)JMP32((uptr)Dispatcher - ( (uptr)x86Ptr + 5 ));
}
...
void iFlushCall(int flushtype)
{
    if( flushtype & FLUSH_FREE_VU0 ) {
        // free all VU0 related registers
        int i;
        for(i = 0; i < ARRAYSIZE(x86regs); ++i) {
            if( !(x86regs[i].type&X86TYPE_VU1) && X86_ISVI(x86regs[i].type) ) {
                _freeX86reg(i);
            }
        }

        for(i = 0; i < ARRAYSIZE(xmmregs); ++i) {
            if( xmmregs[i].type == XMMTYPE_VFREG && xmmregs[i].VU == 0 ) {
                _freeXMMreg(i);
            }
        }
    }

    if( flushtype & FLUSH_FREE_ALLX86 )
        _freeX86regs();
    if( flushtype & FLUSH_FREE_TEMPX86 )
        _freeX86tempregs();
    if( flushtype & FLUSH_FLUSH_ALLX86 )
        _flushX86regs();
...
void recMFSA( void ) 
{
	...
	if( (mmreg = _checkXMMreg(XMMTYPE_GPRREG, _Rd_, MODE_WRITE)) >= 0 ) {
		SSE_MOVLPS_M64_to_XMM(mmreg, (uptr)&cpuRegs.sa);
	}
    else {
        MOV64MtoR(RAX, (uptr)&cpuRegs.sa);
		_deleteEEreg(_Rd_, 0);
		MOV64RtoM((uptr)&cpuRegs.GPR.r[_Rd_].UL[0], RAX);
    }
	//else {
    //    mmreg = _allocX86reg(-1, X86TYPE_GPR, _Rd_, MODE_WRITE);
    //    MOV64MtoR(mmreg, (uptr)&cpuRegs.sa);
    //}
}
...
//
//	if( !delayslot ) {
//		CMP32ItoM((uptr)&cpuRegs.pc, s_pCurBlockEx->startpc);
//		j8Ptr[0] = JB8(0);
//		CMP32ItoM((uptr)&cpuRegs.pc, pc);
//		j8Ptr[1] = JA8(0);
//		j8Ptr[2] = JMP8(0);
	...
	for(i = 0; i < X86REGS; ++i) {
		if( x86regs[i].inuse ) {
            if( x86regs[i].type == X86TYPE_GPR ) {
                count = _recIsRegWritten(g_pCurInstInfo, (s_nEndBlock-pc)/4 + 1, XMMTYPE_GPRREG, x86regs[i].reg);
                if( count > 0 ) x86regs[i].counter = 1000-count;
                else x86regs[i].counter = 0;
            }
            else x86regs[i].counter = 0;
		}
	}
...
//	}
	...
	if( branch == 2 ) {
		...
		else ADD32ItoM((uptr)&cpuRegs.cycle, s_nBlockCycles*9/8);
	...
	if( !branch ) {
		...
		if( pcurblock->startpc == startpc ) {
			...
			*ptr = s_pCurBlock->pFnptr - ( (uptr)ptr + 4 );
		}
		else {
